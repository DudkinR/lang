...
<!--  form for create game-->
    <form action="{{ route('game.update',$game->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="name" name="name" value="{{$game->name}}" placeholder="Enter name">
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <textarea class="ckeditor form-control" id="description" name="description" rows="10">{{$game->description}}</textarea>
        </div>
        <div class="form-group">
            <label for="url">Url</label>
            <input type="text" class="form-control" id="url" name="url" value="{{$game->url}}" placeholder="Enter url">
        </div>
        <div class="form-group">
            ...
            <?php
            $parameters= json_decode($game->parameters);
            $num=0;
            ?>
            ...
            <?php $num++;?>
                ...
                {{$key}}
                    ...
                    @if($key=='name')
                    <div class="col-sm-4">
                    <input type="text" class="form-control" id="parameter_{{$key}}_{{$num}}" name="parameter_{{$key}}_{{$num}}" value="{{$value}}" placeholder="Enter parameter name">
                    </div>
                    @elseif($key=='value')
                    <div class="col-sm-4">
                    <input type="text" class="form-control" id="parameter_{{$key}}_{{$num}}" name="parameter_{{$key}}_{{$num}}" value="{{$value}}" placeholder="Enter parameter value">
                    </div>
                    @elseif($key=='type')
                    <div class="col-sm-4">


                    <select class="form-control" id="parameter_{{$key}}_{{$num}}" name="parameter_{{$key}}_{{$num}}">
                    <option value="string" @if($value=='string') selected @endif>string</option>
                    <option value="int" @if($value=='int') selected @endif>int</option>
                    <option value="bool" @if($value=='bool') selected @endif>bool</option>
                    <option value="json" @if($value=='json') selected @endif>json</option>
                    </select>
                    </div>
                    @endif
                    </div>
                @endforeach
            @endforeach
                    <input type="text" id="count_parameters" name="count_parameters" value="{{$num}}">
